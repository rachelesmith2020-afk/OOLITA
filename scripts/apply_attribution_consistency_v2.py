...
for rel, marker, final, old_role, new_role in (
    ("index.html", "Raquel Costantini hizo el laberinto", HOME_ES, "Vestini Tribe — editorial", "Vestini Tribe — editorial del libro"),
    ("en/index.html", "Raquel Costantini made the labyrinth", HOME_EN, "Vestini Tribe — publisher", "Vestini Tribe — book publisher"),
):
    path = ROOT / rel
    text = path.read_text(encoding="utf-8")
    text, found = replace_paragraph_containing(text, marker, final)
    if not found:
        # The earlier audit form may still be present if the voice layer did not touch it.
...
for rel, marker, final in (
    ("sobre-oolita/index.html", "OOLITA tiene tres formas:", ABOUT_ES),
    ("en/about/index.html", "OOLITA has three forms:", ABOUT_EN),
):
    path = ROOT / rel
    text = path.read_text(encoding="utf-8")
    text, found = replace_paragraph_containing(text, marker, final)
    if not found:
        fallback = "OOLITA es el nombre público" if rel == "sobre-oolita/index.html" else "OOLITA is the public identity"
        text, found = replace_paragraph_containing(text, fallback, final)
    if not found:
