#!/usr/bin/env python3
"""Final native-Spanish editorial pass for OOLITA.

This pass is deliberately narrow. It corrects reader-facing Spanish that is
unidiomatic, ambiguous or inconsistent with the final bilingual book while
preserving the established OOLITA voice, structure, English copy and archived
poster text. It is designed to run after the broader content/geology passes and
to fail closed if the expected copy state drifts.
"""
...
changed = 0
for rel, old, new, label in rules:
    changed += int(replace_state(rel, old, new, label))

# The final book excerpt is an explicit publication invariant: the website must
# quote the corrected Spanish interior, not an earlier translation state.
...
print(f"OOLITA native Spanish editorial pass complete: {changed} edit(s) applied; protected anchors intact.")
