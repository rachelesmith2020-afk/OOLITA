...
runs idempotently after the broader voice audit.
"""
...
for old, new in rules:
    if new in text:
        continue
    if old not in text:
        raise SystemExit(f"Unexpected Spanish Editions wording state: {old[:90]}")
...
for _, new in rules:
    if new not in verified:
        raise SystemExit("Spanish Editions voice replacement did not persist")

print(f"Spanish Editions voice audit passed: {changed} edit(s) applied.")
